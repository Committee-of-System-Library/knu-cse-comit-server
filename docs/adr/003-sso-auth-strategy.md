...
4. 프론트 — Client Secret(HMAC-SHA256)으로 JWT 서명 검증
5. 이후 API 호출: Authorization: Bearer <SSO JWT>
6. comit-server — 서명 검증 후 클레임 추출
```
...
## 결정 2: EXTERNAL 사용자 403 차단

`userType == EXTERNAL`이면 서비스 이용을 거부한다.

- 응답: `403 Forbidden` + 전용 에러코드 `ACCESS_DENIED_FOR_EXTERNAL_USER`
- 401은 "인증 안 됨"의 의미라 부적절하다. SSO 인증은 성공했지만 서비스 정책상 이용 불가이므로 403이 맞다.
- 프론트는 이 에러코드를 감지해 "외부인 이용 불가" 안내 페이지로 이동한다.
...
**수집 항목**
- SSO에서 pre-fill: 이름, 이메일, 학번, userType, major
- 사용자 직접 입력: 닉네임, 연락처, 세부전공 확인/수정, 개인정보 수집·이용 동의

**흐름**
```
미등록 상태로 API 호출
→ 404 + MEMBER_NOT_REGISTERED 에러코드
→ 프론트 → /signup 페이지로 이동 (SSO 클레임으로 폼 pre-fill)
→ 추가 입력 완료 → POST /members/signup
→ 이후 정상 사용
```
...
> `major` 클레임은 현재 `MemberPrincipal`에 미포함. 회원가입 플로우 구현 시 추가 예정.
