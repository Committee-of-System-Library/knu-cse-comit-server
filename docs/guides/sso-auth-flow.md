...
3. KNU 인증 서버 로그인 URL로 302 리다이렉트합니다.
   ```
...
KNU 인증 서버가 직접 이 URL을 호출합니다. 백엔드가 처리를 완료하면 자동으로 `frontendSuccessUrl`로 리다이렉트되므로,
프론트엔드는 그 URL에서 로그인 완료 상태로 시작하면 됩니다.
...
백엔드는 로그인 성공 시 `frontendSuccessUrl`로 리다이렉트합니다.
이 URL을 진입 경로로 사용하면 됩니다.

```javascript
// 예: frontendSuccessUrl = "http://localhost:5173/auth/success"
// 해당 페이지 컴포넌트에서 로그인 완료 처리
function AuthSuccessPage() {
  useEffect(() => {
    // 쿠키(COMIT_SSO_TOKEN)가 이미 심겨 있으므로
    // 바로 API 호출이 가능한 상태
    navigate("/home");
  }, []);
}
