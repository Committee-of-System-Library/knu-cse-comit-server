...
- 운영 원칙
  - `/opt/docker/env/comit.env`는 staging 상시 운영값 source of truth로 사용한다.
  - `/home/yujihun20251/comit.compose.override.yml`은 개인 Vercel smoke test, 임시 CORS 확장, 긴급 우회 같은 일시 변경에만 사용한다.
  - 같은 키를 `comit.env`와 override에 동시에 두지 않는다.
...
- [x] `staging` 프로필 파일 추가
- [x] 운영 필수 env 목록 정리 (`/opt/docker/env/comit.env` source of truth)
- [x] 위험한 기본값 제거 또는 staging/prod에서 명시적 덮어쓰기 (`DDL_AUTO:none` 기본값 적용됨)
- [x] 인증 브리지 플래그 기반 활성화로 전환 (`COMIT_AUTH_BRIDGE_ENABLED`)
- [ ] staging 보호 정책 결정 (Basic Auth / IP allowlist 미적용)
- [x] backend deploy workflow를 `pull + compose up -d`까지 확장
- [x] GHCR 이미지명 `knu-cse-comit-server`로 정리
- [x] `/opt/docker/compose/docker-compose.services.yml`에 backend 등록
- [x] `/opt/docker/nginx/conf.d/comit.conf` 작성
- [x] staging DB와 계정 준비
- [x] `/opt/docker/env/comit.env` 작성
- [x] Pinpoint agent 연결 (`comit.env`의 `JAVA_OPTS`로 연결됨, `applicationName=comit-backend`, `agentId=comit-stg-01`)
- [ ] healthcheck rollout 확인 (actuator 의존성 추가 완료, curl 설치 완료 — sidowi compose `up -d` 후 `docker inspect` Health 상태 검증 필요)
- [x] backend runner 등록 (`Comit_BE` — `/home/yujihun20251/actions-runner/comit-backend/`, online)
- [ ] 첫 자동 배포 성공 확인
- [x] staging smoke test 통과 (API docs, CORS, SSO member 생성 검증 완료)
