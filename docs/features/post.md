...
### 5.5 목록 본문 미리보기 규칙
- `GET /posts` 목록 응답은 전체 본문 대신 `contentPreview`를 반환한다.
- `contentPreview`는 본문을 trim한 뒤 줄바꿈·탭·중복 공백을 하나의 공백으로 정리해 만든다.
- `contentPreview`는 최대 80자까지만 제공하고, 초과 시 뒤에 `...`를 붙인다.
