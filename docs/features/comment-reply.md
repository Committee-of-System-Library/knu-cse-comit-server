...
댓글에는 "도움이요" 리액션을 달 수 있고, 도움이요 수가 높은 댓글이 상단에 노출된다.
...
`CommentHelpful` 엔티티는 (comment_id, member_id) unique constraint로 중복 도움이요를 막는다.
`Comment`와 `@ManyToOne` 관계를 맺지 않고 raw `Long`으로 저장한다.
→ helpful 여부 조회 시 Comment 엔티티를 불필요하게 로딩하지 않기 위함.
...
N+1 없이 2번의 쿼리로 전체 스레드를 구성한다.

```
쿼리 1: findActiveTopLevelByPostId   → 최상위 댓글 목록 (JOIN FETCH member, helpfulCount DESC 정렬)
쿼리 2: findActiveRepliesByPostId    → 해당 게시글의 대댓글 전체 (JOIN FETCH member, JOIN FETCH parentComment)
쿼리 3: findHelpfulCommentIds        → 현재 요청자가 helpful 누른 댓글 ID 목록
...
`helpfulIds`를 `Set<Long>`으로 변환해 `contains()` 조회를 O(1)로 처리한다.
...
```
INSERT IGNORE INTO comment_helpful (comment_id, member_id, created_at) VALUES (?, ?, NOW())
  → affected rows = 1: 최초 helpful → incrementHelpfulCount
  → affected rows = 0: 이미 helpful 상태 → DELETE + decrementHelpfulCount
```

`decrementHelpfulCount` 쿼리에 `AND c.helpfulCount > 0` 조건을 포함해 음수 방지.
...
### `CommentHelpful`에 raw Long ID 저장

**결정:** `CommentHelpful.commentId`, `memberId`를 `@ManyToOne` FK 없이 Long으로 저장한다.

**이유:**
helpful 조회 패턴은 항상 "이 memberId가 이 commentId들에 helpful 눌렀는가"이다.
